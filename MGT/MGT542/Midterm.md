# MGT542 Midterm Study Guide

## 1. Message Intent Types

### Informational

Purpose: Deliver facts or updates without attempting to influence
attitudes or behavior.

Characteristics: 
- Neutral tone 
- Fact‑based 
- Focus on clarity and
accuracy

Risks: 
- Information overload 
- Low engagement 
- Misinterpretation without context

------------------------------------------------------------------------

### Persuasive

Purpose: Influence beliefs, attitudes, or decisions.

Characteristics: 
- Uses logic (logos), credibility (ethos), and emotion (pathos) 
- Structured arguments 
- Clear call to action

Risks: 
- Audience skepticism 
- Weak evidence damaging credibility

------------------------------------------------------------------------

### Directive

Purpose: Tell the audience what actions to take.

Characteristics: 
- Clear instructions 
- Often hierarchical 
- Focus on operational action

Risks: 
- Resistance if tone feels authoritarian 
- Confusion if instructions lack clarity

------------------------------------------------------------------------

### Relational

Purpose: Maintain or improve relationships and trust.

Examples: 
- Feedback 
- Apologies 
- Recognition

Risks: 
- Emotional escalation 
- Misinterpreted tone

------------------------------------------------------------------------

### Entertainment / Engagement

Purpose: Capture attention and create emotional engagement.

Characteristics: 
- Storytelling 
- Emotion-driven messaging 
- Memorable presentation

Risks: 
- Message overshadowed by entertainment

------------------------------------------------------------------------

# 2. Sensitive vs Non‑Sensitive Communication

### Non‑Sensitive Messages

Examples: 
- Routine updates 
- Task assignments 
- Scheduling information

Characteristics: 
- Low emotional impact 
- Efficiency-focused

Risks: 
- Message fatigue 
- Ignored communications

------------------------------------------------------------------------

### Sensitive Messages

Examples: 
- Performance feedback 
- Layoffs 
- Conflict discussions

Characteristics: 
- High emotional stakes 
- Requires careful tone and delivery

Risks: 
- Defensive reactions 
- Emotional escalation 
- Misinterpretation

------------------------------------------------------------------------

### Causes of Misidentifying Sensitivity

Common causes: 
- Sender perspective bias 
- Cultural differences 
- Overreliance on written communication 
- Poor audience analysis

Prevention: 
- Evaluate emotional impact 
- Consider audience identity and status 
- Default to higher sensitivity when uncertain

------------------------------------------------------------------------

# 3. Core Dimensions of Effective Communication

### Clarity

-   Easy to understand
-   Unambiguous instructions
-   Simple language

### Relevance

-   Directly connected to audience tasks
-   Timely and role‑specific

### Consistency

-   Messages aligned across leadership and channels

### Accessibility

-   Easy to access when needed
-   Channels appropriate for workforce

### Trust & Credibility

-   Transparent leadership
-   Fact‑based communication

### Feedback & Dialogue

-   Two‑way communication
-   Mechanisms for questions and feedback

------------------------------------------------------------------------

# 4. Writing Styles

### Formal Writing

Characteristics: 
- Structured grammar 
- Objective tone 
- Professional vocabulary

Examples: 
- Academic writing 
- Legal documents 
- Executive reports

Risks: 
- Complexity reducing readability

------------------------------------------------------------------------

### Informal Writing

Characteristics: 
- Conversational tone 
- Short sentences 
- Flexible structure

Examples: 
- Team messages 
- Social media 
- Internal updates

Risks: 
- Reduced professionalism 
- Tone misinterpretation

------------------------------------------------------------------------

# 5. Tone, Voice, and Word Choice

### Tone

Signals emotional intent.

Examples: 
- Supportive 
- Neutral 
- Urgent 
- Authoritative

------------------------------------------------------------------------

### Voice

Active voice improves clarity.

Example:

Active: Manager approved the request.

Passive: The request was approved.

------------------------------------------------------------------------

### Conciseness Techniques

-   Remove filler words
-   Eliminate redundancy
-   Replace vague phrases
-   Avoid softened language

Example:

Wordy: We are currently in the process of evaluating options.

Concise: We are evaluating options.

------------------------------------------------------------------------

# 6. The Four S's of Effective Communication

### Structure

Organizes ideas logically and reduces cognitive load.

### Style

Tone, voice, and professionalism of the message.

### Substance

Core message and supporting evidence.

### Stickiness

How memorable the message is.

------------------------------------------------------------------------

# 7. Sticky Messages - SUCCESs Framework

**Simple**:  Identify the core idea
**Unexpected**:  Break patterns to gain attention
**Concrete**:  Use vivid examples
**Credible**:  Provide believable evidence
**Emotional**:  Connect to audience values
**Stories**:  Use narratives to motivate action

------------------------------------------------------------------------

# 8. Strategic Story Types

**Story of Me**: personal credibility and motivation
**Story of Us**: shared identity and collective values
**Story of an Idea**:  origin or importance of a concept
**Story of Others**:  examples or case studies
**Story of the Audience**:  audience positioned as the hero

------------------------------------------------------------------------

# 9. Brain Regions Involved in Communication

### Language Processing

**Broca's Area**:  speech production
**Wernicke's Area**:  language comprehension
**Primary Auditory Cortex**:  processing speech sounds
**Visual Cortex**:  processing text and visual cues
**Motor Cortex**:  speaking, typing, gesturing

------------------------------------------------------------------------

### Emotional & Social Processing

**Amygdala**:  emotional threat detection
**Insula**:  empathy and internal emotional awareness
**Anterior Cingulate Cortex**:  conflict detection and emotional
regulation
**Medial Prefrontal Cortex**:  social reasoning

------------------------------------------------------------------------

### Executive Communication Control

**Dorsolateral Prefrontal Cortex**:  reasoning and planning
**Inferior Frontal Gyrus**:  tone regulation and impulse control
**Supplementary Motor Area**:  coordination of speech and gestures

------------------------------------------------------------------------

# 10. Brain Response by Message Type

**Informational**:  language comprehension networks
**Persuasive**:  logic and emotional systems
**Directive**:  motor planning and control systems
**Relational**:  social cognition networks
**Entertainment**:  reward systems

Key neurochemicals:

**Dopamine**:  reward and motivation
**Oxytocin**:  social bonding and trust

------------------------------------------------------------------------

# 11. Hofstede Cultural Dimensions

### Power Distance

Acceptance of unequal power distribution.

Low: US, UK
High: China, India

------------------------------------------------------------------------

### Individualism vs Collectivism

Individualism: Focus on personal achievement.

Collectivism: Focus on group harmony.

Examples:

Individualist: US, Australia
Collectivist: China, Japan

------------------------------------------------------------------------

### Uncertainty Avoidance

High: Preference for rules and structure.

Low: Comfort with ambiguity.

Examples:

High: Japan, Greece
Low: US, Singapore

------------------------------------------------------------------------

### Masculinity vs Femininity

Masculine cultures emphasize: 
- Competition 
- Achievement

Feminine cultures emphasize: 
- Cooperation 
- Quality of life

------------------------------------------------------------------------

### Long‑Term Orientation

Long-term focus: China, Japan

Short-term focus: United States

------------------------------------------------------------------------

### Indulgence vs Restraint

Indulgent cultures emphasize enjoyment and leisure.

Restrained cultures emphasize discipline and social norms.

------------------------------------------------------------------------

# 12. CAGE Framework

Used to analyze international business and communication differences.

### Cultural Distance

Language, norms, values.

### Administrative Distance

Legal systems, government structures, political relationships.

### Geographic Distance

Physical distance, time zones, transportation infrastructure.

### Economic Distance

Income levels, market development, purchasing power.
